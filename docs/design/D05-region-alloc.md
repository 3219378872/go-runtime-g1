...
ID: D05。上游：S03。实现：[M02 dense-refill](../impl/M02-fork-map.md#dense-refill)。证据：[E02 ra1](../evidence/E02-bench-matrix.md#ra1)。
...
- STW-only 的 `scanTag` 读安全：refill 永不在 STW 跑。推回保持 swept 集不变式；返回 span 仍满足原 free-space 契约。`g1gc=0` 时跳过排名，沿用原 partialSwept 首 pop 路径。
