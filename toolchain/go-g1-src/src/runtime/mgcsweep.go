...
func (h *mheap) nextSpanForSweep() *mspan {
	sg := h.sweepgen
	if debug.g1gc != 0 && debug.g1gcset != 0 {
		if s := g1gcNextCandidate(sg); s != nil {
			return s
		}
	}
...
func (sl *sweepLocked) sweep(preserve bool) bool {
	...
	if s.freeindex < s.nelems {
		...
		if (*s.gcmarkBits.bytep(obj / 8)&^*s.allocBits.bytep(obj / 8))>>(obj%8) != 0 {
			if debug.g1evac > 1 {
				print("g1evac zombie span base=", hex(s.base()), " span=", s, " elemsize=", s.elemsize, " nelems=", s.nelems, " alloc=", s.allocCount, " freeindex=", s.freeindex, " sweepgen=", s.sweepgen, " heapSweepgen=", sweepgen, "\\n")
			}
			s.reportZombies()
		}
