...
func gcMarkTermination(stw worldStop) {
	...
	systemstack(func() {
		...
		if debug.g1gc != 0 && debug.g1evac != 0 && g1gcCycleStarted != 0 {
			g1gcFinalizeEvacuation()
			work.stackRoots = nil
		}
		if debug.g1gc != 0 && g1gcCycleStarted != 0 {
			g1gcEndCycle()
			g1gcCycleStarted = 0
		}
	})
	...
	for _, lc := range faultList {
		if debug.g1evac > 1 {
			print("g1evac fault span=", lc.mspan, " base=", hex(lc.mspan.base()), " user=", lc.mspan.isUserArenaChunk, " npages=", lc.mspan.npages, " x=", hex(uintptr(lc.x)), "\\n")
		}
...
func gcBgMarkWorker(ready chan struct{}) {
	...
	node := &(*gcBgMarkWorkerNodePadded)(persistentalloc(unsafe.Sizeof(gcBgMarkWorkerNodePadded{}), tagAlign, &memstats.gcMiscSys)).gcBgMarkWorkerNode
	gp.m.preemptoff = ""
...
func gcMark(startTime int64) {
	...
	if debug.g1gc == 0 || debug.g1evac == 0 {
		work.stackRoots = nil
	}
