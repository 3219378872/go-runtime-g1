...
func writebarrier(f *Func) {
	...
	const maxEntries = 8
	// wbBufEntries is the distance from wbBuf.buf to the parallel slot
	...
	FindSeq:
		...
		addEntry := func(pos src.XPos, ptr, slot *Value) {
			writes = append(writes, write{ptr: ptr, slot: slot, pos: pos})
		...
		for _, w := range stores {
			...
			slotPtr := ptr
			if slotPtr == nilcheck {
				slotPtr = nilcheckThen
			}
			if needWBsrc(val) {
				// Keep one entry per store so owner slot metadata remains
				// complete when the same pointer is installed twice.
				addEntry(pos, val, slotPtr)
			}
			if !dsts.contains(ptr.ID) && needWBdst(ptr, w.Args[2], zeroes) {
				...
				oldVal := bThen.NewValue2(pos, OpLoad, types.Types[types.TUINTPTR], slotPtr, memThen)
				// Save old value to write buffer.
				addEntry(pos, oldVal, slotPtr)
			}
