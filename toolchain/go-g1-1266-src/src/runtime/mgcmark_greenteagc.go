...
//
// gcUsesSpanInlineMarkBits(s.elemsize) must be true.
// g1gcClearInlineMarks clears the mark and scan state stored in a span's
// inline mark bits. Evacuation calls it on source spans whose live objects
// moved to destinations, so the stale marks cannot look like zombies at the
// next sweep.
func g1gcClearInlineMarks(s *mspan) {
	imb := s.inlineMarkBits()
	for i := range imb.marks {
		imb.marks[i] = 0
	}
	for i := range imb.scans {
		imb.scans[i] = 0
	}
}

// g1gcCountInlineMarks counts the marked objects in a span whose marks are
// stored in the span's inline mark bits. It is called at mark termination
// before moveInlineMarks merges them into gcmarkBits during sweep.
func g1gcCountInlineMarks(s *mspan) uint64 {
	imb := s.inlineMarkBits()
	markBytes := divRoundUp(uintptr(s.nelems), 8)
	var count uint64
	for i := uintptr(0); i < markBytes; i++ {
		count += uint64(sys.OnesCount64(uint64(imb.marks[i])))
	}
	return count
}
...
func scanObjectSmall(spanBase, b, objSize uintptr, gcw *gcWork) {
	...
	for _, p := range gcw.ptrBuf[:nptrs] {
		...
		if evacActive {
			if target := spanOfHeap(p); target != nil {
				g1gcRecordInboundActive(gcw, owner, target, p)
			}
		}
		if !tryDeferToSpanScan(p, gcw) {
			if obj, span, objIndex := findObject(p, 0, 0); obj != 0 {
...
func scanObject(b uintptr, gcw *gcWork) {
	...
	for {
		...
		if obj != 0 && obj-b >= n {
			...
			} else if !tryDeferToSpanScan(obj, gcw) {
