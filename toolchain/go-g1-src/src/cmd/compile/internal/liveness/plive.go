...
func (lv *Liveness) markUnsafePoints() {
	...
	for _, b := range lv.f.Blocks {
		for _, v := range b.Values {
			...
			// Find the write barrier decision block. The G1 slot path can
			// add extra predecessors to the join while preserving the
			// original write-barrier on/off decision as a root.
			if len(m.Args) < 2 {
				lv.f.Fatalf("phi before write barrier end mark has %d args, want 2", len(m.Args))
			}
			var decisionBlock *ssa.Block
			var load *ssa.Value
			findWBLoad := func(v *ssa.Value) *ssa.Value {
				for {
					if v == nil {
						return nil
					}
			...
			// Walk backwards from the join when later CFG simplification has
			// folded either side of the diamond into the join itself.
			seenBack := make(map[*ssa.Block]bool)
			pendingBack := make([]*ssa.Block, 0, len(b.Preds))
			for _, e := range b.Preds {
				pendingBack = append(pendingBack, e.Block())
			}
			for len(pendingBack) != 0 && decisionBlock == nil {
				x := pendingBack[0]
				pendingBack = pendingBack[1:]
				if seenBack[x] {
					continue
				}
				seenBack[x] = true
				if candidate := findWBLoad(x.Controls[0]); candidate != nil {
					decisionBlock = x
					load = candidate
					break
				}
			...
			if load == nil {
				load = findWBLoad(decisionBlock.Controls[0])
				if load == nil {
					decisionBlock.Controls[0].Fatalf("load of write barrier flag not from correct global: %s", decisionBlock.Controls[0].LongString())
				}
			}
			...
			// Mark the write barrier on/off paths as unsafe. The enabled
			// path may contain nested G1 slot branches, so walk it until the
			// common join instead of marking only one block.
			seen := make(map[*ssa.Block]bool)
			seen[decisionBlock] = true
			pending := make([]*ssa.Block, 0, 2)
			for _, e := range decisionBlock.Succs {
				if e.Block() != b {
					pending = append(pending, e.Block())
				}
			}
			for len(pending) != 0 {
				...
				for _, value := range x.Values {
					lv.unsafePoints.Set(int32(value.ID))
				}
