...
type wbBuf struct {
	...
	// slots mirrors buf. The compiler and bulk barrier paths record the heap
	// slot which produced each value so G1 can maintain an inbound owner index.
	// It is uintptr metadata and must remain outside the Go pointer bitmap.
	slots [wbBufEntries]uintptr
}
...
// setSlot records the owner slot corresponding to a value returned by get1 or
// get2. The caller must provide a pointer into b.buf.
//
//go:nosplit
func (b *wbBuf) setSlot(p, slot uintptr) {
	start := uintptr(unsafe.Pointer(&b.buf[0]))
	index := (p - start) / unsafe.Sizeof(b.buf[0])
	if index >= uintptr(len(b.slots)) {
		throw("runtime: write barrier slot metadata overflow")
	}
	b.slots[index] = slot
}
...
func wbBufFlush1(pp *p) {
	...
	if g1EvacIndexActive != 0 {
		for i, ptr := range ptrs {
			slot := ptrSlots[i]
			if slot != 0 {
				g1gcRecordInboundSlotActive(gcw, slot, ptr)
